...
def HOBOgrab():
	for serial in serials: 
		...
		f_latest = open( serial+ "_" + time.time() + ".csv", 'wt')
		try:
		...
		try:
			filename1 = serial +"_"+time.time()+".csv" 
			filename2 = serial+"_"+(time.time()*60*60*60)+".csv"
			recent  = csv.read(filename1, "r")
			last = csv.read(filename2, "r")	 
